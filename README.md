...
本项目使用阿里云函数计算（白嫖）来进行一次URL转发，并缓存订阅数据，如源订阅地址访问失败，则使用缓存来进行返回，完全避免偶尔订阅失败的问题。

函数计算调用消耗有非常大的免费额度，是完全用不完的，你只需要付费0.5元/GB的外网流量即可，一个月大概消耗几M的流量，1块钱可以用十几年，无限接近于白嫖。

## 部署项目需要什么

无技术含量，也不用将订阅地址交给别人，你只需要有个阿里云账号（淘宝账号），然后开通一个对象存储桶，然后开通一个函数计算即可。
